class FileTree {
  ...
  _create() {
    ...
    this.container.style.setProperty('--aifr-console-width', `${this.bridgeConsoleWidth}px`);
    this.shadowRoot = this.container.attachShadow({ mode: 'closed' });

    this.shadowRoot.innerHTML = `
      <style>${this._getStyles()}</style>
      <div class="aifr-bridge-console" id="bridge-console">
        <div class="aifr-console-resizer" id="bridge-console-resizer" title="拖动调整宽度"></div>
        <div class="aifr-console-header">
          ...
          <div class="aifr-console-actions">
            ...
            <button class="aifr-btn" id="bridge-sidebar-toggle" title="隐藏右侧目录面板">隐藏目录</button>
        ...
        <div class="aifr-console-direct">
          <textarea id="bridge-console-direct-input" placeholder="直接对 Claude Code 说... Enter 发送，Shift+Enter 换行" rows="3"></textarea>
      ...
      <div class="aifr-sidebar" id="sidebar">
        ...
        <div class="aifr-bridge" id="bridge-panel">
          ...
          <div class="aifr-bridge-row" style="margin-top:6px;">
            <label style="font-size:11px;color:#e2e8f0;display:flex;align-items:center;gap:4px;cursor:pointer;">
          ...
          <div class="aifr-bridge-direct" style="margin-top:8px;">
            <div style="font-size:11px;color:#cba6f7;margin-bottom:4px;display:flex;align-items:center;gap:6px;">
              ...
              <span style="color:#c8ceda;font-size:10px;">Enter 发送 / Shift+Enter 换行</span>
            </div>
            <textarea id="bridge-direct-input" placeholder="任何时候在这里打字，绕过网页 AI 直接让 Claude Code 执行..." rows="2" style="
              ...
              background:#11111b;color:#e2e8f0;font-size:11px;outline:none;
        ...
        <div class="aifr-status-bar">
          <span class="aifr-status" id="status" title="手动输入路径或选择最近目录"></span>
          <button class="aifr-btn" id="switch-root-btn" title="选择文件夹">📂</button>
        </div>
    ...
    const bridgeSidebarToggle = this.shadowRoot.getElementById('bridge-sidebar-toggle');
    const bridgeNewSession = this.shadowRoot.getElementById('bridge-new-session');
    const bridgeConsoleResizer = this.shadowRoot.getElementById('bridge-console-resizer');
    ...
    bridgeSidebarToggle.addEventListener('click', () => this.toggleDirectoryPanel());
    this._setupBridgeConsoleResize(bridgeConsoleResizer);
    ...
    const statusBar = this.shadowRoot.querySelector('.aifr-status-bar');
    switchRootBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      this._pickRootWithDialog();
    });
    statusBar.addEventListener('click', () => this._showSwitchRootManual());
    ...
    chrome.storage.local.get(['bridgeConsoleWidth'], (data) => {
      if (data.bridgeConsoleWidth) {
        this.setBridgeConsoleWidth(data.bridgeConsoleWidth, { persist: false });
      }
    });
    window.addEventListener('resize', () => {
      this.setBridgeConsoleWidth(this.bridgeConsoleWidth, { persist: false });
    });
  }
  ...
  _getStyles() {
    return `
      .aifr-sidebar {
        ...
        box-shadow: -4px 0 24px rgba(0, 0, 0, 0.4), inset 1px 0 0 rgba(255, 255, 255, 0.06);
        border-left: 1px solid rgba(137, 180, 250, 0.15);
      ...
      .aifr-bridge-console {
        ...
        top: 0;
        right: 300px;
        width: min(var(--aifr-console-width, 520px), calc(100vw - 320px));
        min-width: 320px;
        height: 100vh;
        ...
        transition: transform 0.25s cubic-bezier(0.4, 0, 0.2, 1);
        border-left: 1px solid rgba(137, 180, 250, 0.1);
        border-right: 1px solid rgba(69, 71, 90, 0.4);
      }
      .aifr-bridge-console.visible {
        transform: translateX(0);
      }
      .aifr-bridge-console.sidebar-hidden {
        right: 0;
        width: min(var(--aifr-console-width, 520px), calc(100vw - 20px));
        transform: translateX(100%);
        border-right: none;
      }
      .aifr-bridge-console.sidebar-hidden.visible {
        transform: translateX(0);
      }
      .aifr-console-resizer {
        position: absolute;
        top: 0;
        left: -5px;
        width: 10px;
      ...
      .aifr-console-resizer:hover::after,
      .aifr-bridge-console.resizing .aifr-console-resizer::after {
      ...
      .aifr-console-direct button:hover {
        opacity: 0.9;
      }
      .aifr-console-direct button:active {
      ...
      .aifr-title {
        ...
        font-size: 14px;
        color: #89b4fa;
        text-shadow: 0 0 12px rgba(137, 180, 250, 0.2);
      }
      .aifr-header-actions {
        display: flex;
        gap: 4px;
      ...
      #switch-root-btn:hover {
        background: rgba(49, 50, 68, 0.6);
        border-color: rgba(137, 180, 250, 0.4);
      }
      #switch-root-btn:disabled {
        opacity: 0.75;
        cursor: wait;
      }
      .aifr-tree {
      ...
      .aifr-tree::-webkit-scrollbar-thumb:hover {
        background: rgba(108, 112, 134, 0.6);
      }
      .aifr-node {
        display: flex;
        align-items: center;
  ...
  _setupBridgeConsoleResize(handle) {
    if (!handle) return;

    let latestWidth = this.bridgeConsoleWidth;

    const stopResize = () => {
      const consoleEl = this.shadowRoot.getElementById('bridge-console');
      if (consoleEl) consoleEl.classList.remove('resizing');
      document.removeEventListener('mousemove', onMove);
      document.removeEventListener('mouseup', stopResize);
      document.documentElement.style.cursor = '';
      document.body.style.userSelect = '';
      this.setBridgeConsoleWidth(latestWidth);
    };

    const onMove = (e) => {
      const rightReserve = this.visible ? 300 : 0;
      latestWidth = window.innerWidth - rightReserve - e.clientX;
      this.setBridgeConsoleWidth(latestWidth, { persist: false });
    };

    handle.addEventListener('mousedown', (e) => {
      e.preventDefault();
      const consoleEl = this.shadowRoot.getElementById('bridge-console');
      if (consoleEl) consoleEl.classList.add('resizing');
      latestWidth = this.bridgeConsoleWidth;
  ...
  setBridgeConsoleWidth(width, options = {}) {
    const value = Number(width);
    if (!Number.isFinite(value)) return;

    const rightReserve = this.visible ? 300 : 0;
    const maxWidth = Math.max(320, window.innerWidth - rightReserve - 20);
    const minWidth = Math.min(360, maxWidth);
    const clamped = Math.round(Math.min(Math.max(value, minWidth), maxWidth));

    this.bridgeConsoleWidth = clamped;
    if (this.container) {
      this.container.style.setProperty('--aifr-console-width', `${clamped}px`);
    }

    if (options.persist !== false) {
      chrome.storage.local.set({ bridgeConsoleWidth: clamped });
    }
  }
  ...
  setDirectoryPanelVisible(visible) {
    this.visible = Boolean(visible);
    const sidebar = this.shadowRoot.getElementById('sidebar');
    const consoleEl = this.shadowRoot.getElementById('bridge-console');
    const toggle = this.shadowRoot.getElementById('bridge-sidebar-toggle');

    if (sidebar) sidebar.classList.toggle('visible', this.visible);
    if (consoleEl) consoleEl.classList.toggle('sidebar-hidden', !this.visible);
    if (toggle) {
      toggle.textContent = this.visible ? '隐藏目录' : '显示目录';
      toggle.title = this.visible ? '隐藏右侧目录面板' : '显示右侧目录面板';
    }
    this.setBridgeConsoleWidth(this.bridgeConsoleWidth, { persist: false });
  }

  toggleDirectoryPanel() {
    this.setDirectoryPanelVisible(!this.visible);
  }
  ...
  async show() {
    this.setDirectoryPanelVisible(true);
    ...
    if (window.__aifrBridge && window.__aifrBridge.syncStatus) {
      tasks.push(window.__aifrBridge.syncStatus().catch(() => {}));
    }
  ...
  hide() {
    this.setDirectoryPanelVisible(false);
  }
      ...
      tree.innerHTML = `
        <div class="aifr-empty">
          ...
          <button id="aifr-select-folder" style="
            ...
            font-weight: 500;
          ">${btnText}</button>
          <p style="margin-top: 8px; font-size: 11px; color: #c8ceda;">
  ...
  async _pickRootWithDialog() {
    const mode = this.fileAccess.getMode();

    if (mode === 'native' && typeof this.fileAccess.pickRoot === 'function') {
      const switchRootBtn = this.shadowRoot.getElementById('switch-root-btn');
      const originalText = switchRootBtn.textContent;
      switchRootBtn.disabled = true;
      switchRootBtn.textContent = '...';
      this.showToast('请选择工作目录...');
      try {
        const resp = await this.fileAccess.pickRoot();
        if (resp.success) {
        ...
        this.showToast(resp.error || '无法打开文件夹选择器，请手动输入路径', 'error');
        await this._showSwitchRootManual();
      } catch (err) {
        this.showToast(`无法打开文件夹选择器: ${err.message || err}`, 'error');
        await this._showSwitchRootManual();
      } finally {
        switchRootBtn.disabled = false;
        switchRootBtn.textContent = originalText;
      }
      return;
    }
  ...
  async _showSwitchRootManual() {
    ...
    try {
      ...
      if (resp.success && resp.roots && resp.roots.length > 0) {
        recentHtml = `<div style="margin-top:10px;font-size:11px;color:#c8ceda;">最近使用：</div>` +
          resp.roots.map(r => `<div class="aifr-recent-root" data-path="${r.replace(/"/g, '&quot;')}" style="
            padding:5px 8px;margin-top:4px;border-radius:4px;cursor:pointer;font-size:12px;color:#e2e8f0;
    ...
    tree.innerHTML = `
      <div class="aifr-empty">
        ...
        <input type="text" id="aifr-switch-input" value="${(this.fileAccess.rootDir || '').replace(/"/g, '&quot;')}" style="
          ...
          background:#11111b;color:#e2e8f0;font-size:12px;outline:none;box-sizing:border-box;
        " />
