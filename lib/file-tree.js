class FileTree {
  ...
  _create() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="aifr-sidebar" id="sidebar">
        ...
        <div class="aifr-bridge" id="bridge-panel">
          ...
          <div class="aifr-bridge-row" style="margin-top:6px;">
            <label style="font-size:11px;color:#a6adc8;display:flex;align-items:center;gap:4px;cursor:pointer;">
              <input type="checkbox" id="bridge-debug-window" style="accent-color:#f9e2af;" />
              显示 CMD 调试窗口
            </label>
          </div>
    ...
    // Persist auto-send checkbox
    const bridgeAuto = this.shadowRoot.getElementById('bridge-auto');
    const bridgeDebugWindow = this.shadowRoot.getElementById('bridge-debug-window');
    chrome.storage.local.get(['bridgeAutoSend', 'bridgeDebugWindow'], (data) => {
      if (data.bridgeAutoSend) bridgeAuto.checked = true;
      if (data.bridgeDebugWindow) bridgeDebugWindow.checked = true;
    });
    bridgeAuto.addEventListener('change', () => {
      chrome.storage.local.set({ bridgeAutoSend: bridgeAuto.checked });
    });
    bridgeDebugWindow.addEventListener('change', () => {
      chrome.storage.local.set({ bridgeDebugWindow: bridgeDebugWindow.checked });
    });
  ...
  isBridgeDebugWindowEnabled() {
    const checkbox = this.shadowRoot.getElementById('bridge-debug-window');
  ...
  showBridgeResult(text, onConfirm, source = 'web_ai') {
    const autoCheckbox = this.shadowRoot.getElementById('bridge-auto');
    if (autoCheckbox && autoCheckbox.checked && source === 'web_ai') {
      onConfirm();
